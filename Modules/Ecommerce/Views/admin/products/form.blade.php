...
$deliveryRules = old('delivery_rules', collect($product->delivery_rules ?? [])->implode(PHP_EOL));
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ $isEdit ? route('admin.ecommerce.products.update', $product) : route('admin.ecommerce.products.store') }}">
        ...
        <div class="d-flex commons-ticky-template-toolbar justify-content-between align-items-center flex-wrap gap-2 mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Build products with variants, inventory rules, tax, shipping, and storefront metadata.</p>
            </div>
        ...
        <div class="row g-4">
            <div class="col-lg-8">
                <div class="card mb-4">
                    ...
                    <div class="card-body row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input name="slug" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $product->slug) }}">
                ...
                <div class="card mb-4">
                    <div class="card-header"><h5 class="mb-0">Attributes & Variant Generator</h5></div>
                    <div class="card-body">
                        @if($attributes->isEmpty())
                            <p class="text-muted mb-0">Create attributes first to generate size/color variant combinations.</p>
                        @else
                            <div class="row g-3">
                    ...
                    <div class="card-footer d-flex flex-wrap gap-2">
                        <button type="button" class="btn btn-primary btn-sm" id="generateVariantsBtn">Generate Variant Combinations</button>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="small text-muted mb-0 mt-3">Each row stores its own SKU, price, stock, thresholds, and linked option IDs.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4">
                    <div class="card-header"><h5 class="mb-0">Inventory Rules</h5></div>
                    <div class="card-body">
                        <div class="mb-3">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div>
                            <label class="form-label">Delivery Rules</label>
                            <textarea name="delivery_rules" rows="4" class="form-control" placeholder="One rule per line">{{ $deliveryRules }}</textarea>
                        </div>
                    </div>
                </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function rowHtml(i, variant) {
        ...
        return `<div class="border rounded p-3 mb-3" data-variant-row>
            <div class="d-flex justify-content-between align-items-start gap-2 mb-3">
                <div><div class="fw-semibold">Variant</div><div class="small text-muted">${labels.join(' / ') || 'Manual variant row'}</div></div>
            ...
            <div class="mb-3">${labels.map(label => `<span class="badge bg-light text-dark me-1">${label}</span>`).join('') || '<span class="text-muted small">No attribute options linked.</span>'}</div>
            <div class="row g-3">
                ...
                <div class="col-md-3"><label class="form-label">Size</label><input name="variants[${i}][size]" class="form-control" value="${variant.size || ''}"></div>
                <div class="col-md-3"><label class="form-label">Color</label><input name="variants[${i}][color]" class="form-control" value="${variant.color || ''}"></div>
                <div class="col-md-3 d-flex align-items-end"><div class="form-check form-switch"><input type="hidden" name="variants[${i}][track_inventory]" value="0"><input class="form-check-input" type="checkbox" name="variants[${i}][track_inventory]" value="1" ${(variant.track_inventory ?? 1) ? 'checked' : ''}><label class="form-check-label">Track Inventory</label></div></div>
                <div class="col-md-3 d-flex align-items-end"><div class="form-check form-switch"><input type="hidden" name="variants[${i}][allow_backorder]" value="0"><input class="form-check-input" type="checkbox" name="variants[${i}][allow_backorder]" value="1" ${(variant.allow_backorder ?? 0) ? 'checked' : ''}><label class="form-check-label">Allow Backorder</label></div></div>
            </div>
        </div>`;
    }
    ...
    function buildVariant(combo) {
        ...
        return {
            ...
            size: (combo.find(item => item.attributeName.toLowerCase() === 'size') || {}).optionName || '',
            color: (combo.find(item => item.attributeType === 'color' || item.attributeName.toLowerCase() === 'color') || {}).optionName || ''
        };
    }
    ...
    document.getElementById('addVariantBtn').addEventListener('click', function () {
        appendRow({id:'', sku:baseSku.value || '', price:valuePrice(), stock:0, status:1, track_inventory:1, low_stock_threshold:10, allow_backorder:0, attribute_option_ids:[], label:[], size:'', color:''});
    });
